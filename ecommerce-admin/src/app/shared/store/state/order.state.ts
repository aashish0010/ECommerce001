...
export class OrderState {
  ...
  updateOrderStatus(
    ...
    { id, payload }: UpdateOrderStatusAction,
  ) {
    const statusName = payload?.note || 'pending';
    return this.orderService.updateOrderStatus(id, statusName).pipe(
      tap({
        next: () => {
          const state = ctx.getState();
          const updatedData = state.order.data.map(o =>
            o.id === id ? { ...o, order_status: { ...o.order_status, name: statusName, slug: statusName } } : o,
          );
          ctx.patchState({
            order: { ...state.order, data: updatedData },
          });
        },
        error: err => {
          throw new Error(err?.error?.message);
