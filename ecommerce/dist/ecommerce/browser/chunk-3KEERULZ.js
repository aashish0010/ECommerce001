...
var \u0275StateStream = class _\u0275StateStream extends \u0275OrderedBehaviorSubject {
  state = toSignal(this.pipe(\u0275wrapObserverCalls(untracked2)), {
    manualCleanup: true,
    requireSync: true
  });
  constructor() {
    super({});
    inject2(DestroyRef).onDestroy(() => this.complete());
  }
  /** @nocollapse */
...
var \u0275NgxsActionRegistry = class _\u0275NgxsActionRegistry {
  ...
  constructor() {
    inject2(DestroyRef).onDestroy(() => this._actionTypeToHandlersMap.clear());
  }
  get(type) {
...
var isStateOperator = isFunction2;

// node_modules/@ngxs/store/fesm2022/ngxs-store.mjs
var PluginManager = class _PluginManager {
  plugins = [];
  _parentManager = inject2(_PluginManager, {
    optional: true,
    skipSelf: true
  });
  _pluginHandlers = inject2(NGXS_PLUGINS, {
    optional: true
  });
  constructor() {
...
var Actions = class _Actions extends Observable {
  constructor() {
    const internalActions$ = inject2(InternalActions);
    const internalExecutionStrategy = inject2(InternalNgxsExecutionStrategy);
...
var InternalDispatcher = class _InternalDispatcher {
  _ngZone = inject2(NgZone);
  _actions = inject2(InternalActions);
  _actionResults = inject2(InternalDispatchedActionResults);
  _pluginManager = inject2(PluginManager);
  _stateStream = inject2(\u0275StateStream);
  _ngxsExecutionStrategy = inject2(InternalNgxsExecutionStrategy);
  _injector = inject2(Injector);
  _destroyRef = inject2(DestroyRef);
  /**
   * Dispatches event(s).
   */
...
var NgxsConfig = class _NgxsConfig {
  ...
  static \u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({
    ...
    factory: () => (() => {
      ...
      const config2 = inject2(NGXS_OPTIONS);
...
var InternalStateOperations = class _InternalStateOperations {
  _stateStream = inject2(\u0275StateStream);
  _dispatcher = inject2(InternalDispatcher);
  _config = inject2(NgxsConfig);
  /**
...
var \u0275PROP_GETTER = new InjectionToken(typeof ngDevMode !== "undefined" && ngDevMode ? "PROP_GETTER" : "", {
  providedIn: "root",
  factory: () => inject2(NgxsConfig).compatibility?.strictContentSecurityPolicy ? compliantPropGetter : fastPropGetter
});
...
var NgxsUnhandledActionsLogger = class _NgxsUnhandledActionsLogger {
  ...
  constructor() {
    const options = inject2(NGXS_DEVELOPMENT_OPTIONS);
...
var StateContextFactory = class _StateContextFactory {
  _internalStateOperations = inject2(InternalStateOperations);
  /**
...
var InternalActionHandlerFactory = class _InternalActionHandlerFactory {
  _actions = inject2(InternalActions);
  _stateContextFactory = inject2(StateContextFactory);
  createActionHandler(path, handlerFn, options) {
    ...
    return (action) => {
      ...
      if (isObservable(result)) {
        result = result.pipe(
          mergeMap((value) => isPromise2(value) || isObservable(value) ? value : of(value)),
        ...
        result = result.pipe(
          ...
          finalize(() => {
            if (typeof ngDevMode !== "undefined" && ngDevMode) {
              ...
              stateContext.setState = noop3;
              stateContext.patchState = noop3;
            }
          })
        );
      } else {
...
var StateFactory = class _StateFactory {
  _injector = inject2(Injector);
  _config = inject2(NgxsConfig);
  _actionHandlerFactory = inject2(InternalActionHandlerFactory);
  _actions = inject2(InternalActions);
  _actionResults = inject2(InternalDispatchedActionResults);
  _initialState = inject2(\u0275INITIAL_STATE_TOKEN, {
    optional: true
  });
  _actionRegistry = inject2(\u0275NgxsActionRegistry);
  _propGetter = inject2(\u0275PROP_GETTER);
...
var Store = class _Store {
  ...
  _internalStateOperations = inject2(InternalStateOperations);
  _config = inject2(NgxsConfig);
  _internalExecutionStrategy = inject2(InternalNgxsExecutionStrategy);
  _stateFactory = inject2(StateFactory);
  /**
  ...
  initStateStream() {
    const initialStateValue = inject2(\u0275INITIAL_STATE_TOKEN);
...
function assertRootStoreNotInitialized() {
  const rootStoreGuard = inject2(ROOT_STORE_GUARD);
...
var LifecycleStateManager = class _LifecycleStateManager {
  _store = inject2(Store);
  _internalStateOperations = inject2(InternalStateOperations);
  _stateContextFactory = inject2(StateContextFactory);
  _appBootstrappedState = inject2(\u0275NgxsAppBootstrappedState);
  _abortController = new AbortController();
  _initStateHasBeenDispatched;
  constructor() {
    inject2(DestroyRef).onDestroy(() => this._abortController.abort());
  }
...
function rootStoreInitializer() {
  ...
  const prebootFns = inject2(NGXS_PREBOOT_FNS, {
    optional: true
  }) || [];
  prebootFns.forEach((prebootFn) => prebootFn());
  const factory = inject2(StateFactory);
  const internalStateOperations = inject2(InternalStateOperations);
  inject2(Store);
  inject2(SelectFactory);
  const states = inject2(ROOT_STATE_TOKEN, {
    optional: true
  }) || [];
  const lifecycleStateManager = inject2(LifecycleStateManager);
...
function featureStatesInitializer() {
  ...
  const internalStateOperations = inject2(InternalStateOperations);
  const factory = inject2(StateFactory);
  const states = inject2(FEATURE_STATE_TOKEN, {
    optional: true
  }) || [];
  const lifecycleStateManager = inject2(LifecycleStateManager);
...
}, provideEnvironmentInitializer(() => inject2(NGXS_ROOT_STORE_INITIALIZER))];
var NGXS_FEATURE_ENVIRONMENT_INITIALIZER = [{
  provide: NGXS_FEATURE_STORE_INITIALIZER,
  useFactory: featureStatesInitializer
}, provideEnvironmentInitializer(() => inject2(NGXS_FEATURE_STORE_INITIALIZER))];
...
function getRootProviders(states, options) {
  return [...states, {
    ...
    useFactory: () => {
      const appBootstrappedState = inject2(\u0275NgxsAppBootstrappedState);
...
var ActionDirector = class _ActionDirector {
  _registry = inject2(\u0275NgxsActionRegistry);
  _actionHandlerFactory = inject2(InternalActionHandlerFactory);
  attachAction(stateToken, Action3, handlerFn, options = {}) {
    const actionHandler = this._actionHandlerFactory.createActionHandler(stateToken.getName(), handlerFn, options);
    const detach = this._registry.register(Action3.type, actionHandler);
    return {
      detach
    };
  }
...
function withNgxsPlugin(plugin) {
  return makeEnvironmentProviders([
    ...
    provideEnvironmentInitializer(() => inject2(PluginManager))
  ]);
}
var REGISTERED_PROVIDERS = new InjectionToken("", {
  ...
  factory: () => {
    ...
    inject2(ApplicationRef).onDestroy(() => registeredProviders.clear());
...
@angular/core/fesm2022/rxjs-interop.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-3KEERULZ.js.map
