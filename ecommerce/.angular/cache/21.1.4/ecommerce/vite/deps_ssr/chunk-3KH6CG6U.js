    ...
    return (action) => {
      ...
      if ((0, import_rxjs2.isObservable)(result)) {
        ...
        if (options.cancelUncompleted) {
          ...
          result = result.pipe((0, import_rxjs2.takeUntil)(new import_rxjs2.Observable((subscriber) => {
...
function createPropertySelectors(parentSelector) {
  ...
  return new Proxy({}, {
    get(_target, prop) {
      const selector = cache[prop] || createSelector(
        [parentSelector],
        // Optional chaining is used because the state being selected may not be
        // registered yet — for example, if the selector is called before `provideStates()`.
        (state) => state?.[prop]
...
function withNgxsPendingTasks() {
  return withNgxsPreboot(() => {
    if (false) {
      console.warn("[withNgxsPendingTasks] This setup is recommended for server-side rendering only. Using it in the browser may lead to redundant change detection cycles and degraded performance.");
    }
...
//# sourceMappingURL=chunk-3KH6CG6U.js.map
